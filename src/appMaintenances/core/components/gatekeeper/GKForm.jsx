...
const GKForm = ({ className, onSubmit, style, children }) => {
  // const [valorContextoForm, setValorContextoForm] = useState([]);//Aqui guardaremos cualquier otro factor de contexto que se necesite guardar
   const [valorContextoForm, setValorContextoForm] = useState({
     testInputCollection:[],
     otra:[],//Aqui guardaremos cualquier otro factor de contexto que se necesite guardar
   });
  ...
  useEffect(() => {
    ...
    //Cuando el contexto haya sido llenado entonces enviar:

    if (valorContextoForm.testInputCollection.length >= 1) {
  
      const elementosGKArry = Array.from(elementosGK.current);
      //console.log(valorContextoForm);
    
      GKForm_logic(elementosGKArry, onSubmit, valorContextoForm);
    }
    
  }, [onSubmit, valorContextoForm]); // Lista de dependencias con onSubmit y valorContextoForm
